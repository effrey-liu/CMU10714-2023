...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        qk_d = self.matmul(q, k) / math.sqrt(q_dim)
        if self.causal:
        ...
        probs = self.dropout(self.softmax(qk_d))
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        out = out.transpose(axes=(1, 2)).reshape((batch_size * queries_len, self.num_head * self.dim_head)) # (bs, q_len, d * head)
